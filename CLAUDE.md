...
- **Provider outages speak up** — rate-limit (post-retry) and out-of-credits errors raise `HouseUnavailableError`, which the Watcher turns into a plain channel notice ("out of model credits — someone let Locke know") instead of silent dead air. Other generation errors still degrade to the parser's placeholder path
